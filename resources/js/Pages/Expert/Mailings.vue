<template>
    <div class="flex flex-col gap-8 max-w-2xl">
        <header>
            ...
            <h1 class="text-2xl font-bold text-gray-900">Отправка писем участникам</h1>
        </header>

        <div v-if="$page.props.flash?.success" class="px-5 py-4 bg-emerald-50 border border-emerald-100 rounded-xl text-sm text-emerald-700 font-medium">
        ...
        <div class="bg-white border border-gray-100 rounded-2xl p-8 shadow-sm">
            <form @submit.prevent="submit">
                <div class="space-y-6">
                    <div>
                        ...
                        <div class="space-y-2">
                            <label class="flex items-center gap-3 cursor-pointer">
                                ...
                                />
                                <span class="text-sm text-gray-700">Все участники конференции</span>
                            </label>
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input
                                    type="radio"
                                    ...
                                    class="w-4 h-4 accent-black"
                                />
                                <span class="text-sm text-gray-700">Участники секций</span>
                            </label>
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input
                                    type="radio"
                                    ...
                                    class="w-4 h-4 accent-black"
                                />
                                <span class="text-sm text-gray-700">Выбранные участники</span>
                            </label>
                        </div>
                    </div>

                    <div v-if="form.type === 'sections'" class="pl-7">
                        <p class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Выберите секции</p>
                        ...
                        <p v-if="form.errors.section_ids" class="text-xs text-red-500 mt-1">{{ form.errors.section_ids }}</p>
                    </div>

                    <div v-if="form.type === 'selected'" class="pl-7">
                        <p class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Выберите участников</p>
                        <div class="space-y-1.5 max-h-56 overflow-y-auto pr-1">
                            ...
                            >
                                ...
                                <span class="text-sm text-gray-700">
                                    ...
                                    <span class="text-gray-400">— {{ participant.email }}</span>
                                </span>
                            </label>
                            <p v-if="participants.length === 0" class="text-sm text-gray-400 px-3 py-2">
                                Участников не найдено.
                            </p>
                        </div>
                        <p v-if="form.errors.user_ids" class="text-xs text-red-500 mt-1">{{ form.errors.user_ids }}</p>
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Сообщение</label>
                        <textarea
                            v-model="form.body"
                            rows="6"
